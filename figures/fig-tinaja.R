...
a = fread("zstdcat ../tinaja/results.csv.zst")
...
gg = sort(unique(b$g))
unlink("tinaja*.png")
png("tinaja%03d.png", height=600,width=600)
...
for(gen in gg) {
    ...
    filled.contour(z,col=bcz,levels=keylevels,
        plot.title = {
            title(line=1,main="Tinaja Cave",cex.main=1.2^3)
...
system("convert -delay 10 tinaja*.png tinaja.mp4")
unlink("tinaja*.png")
